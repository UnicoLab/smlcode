...
```bash
scripts/prepare-release.sh 0.18.1 --dry-run     # look at the diff; nothing is committed
scripts/prepare-release.sh 0.18.1               # for real
```
...
For **v0.18.1 specifically**, the version and the changelog entry are already in the tree,
...
```bash
git show --stat HEAD
git tag -v v0.18.1 2>/dev/null || git show v0.18.1 --stat | head
```
...
git push origin v0.18.1          # this is what starts the release
```
...
```
slmcode_0.18.1_darwin_arm64      slmcode_0.18.1_windows_amd64.exe
slmcode_0.18.1_darwin_amd64      slmcode_0.18.1_windows_arm64.exe
slmcode_0.18.1_linux_arm64       install.sh  install.ps1  install.cmd
slmcode_0.18.1_linux_amd64       SHA256SUMS
```
...
curl -fsSLO https://github.com/UnicoLab/smlcode/releases/download/v0.18.1/slmcode_0.18.1_darwin_arm64
shasum -a 256 -c SHA256SUMS --ignore-missing        # must say OK
chmod +x slmcode_0.18.1_darwin_arm64
./slmcode_0.18.1_darwin_arm64 version --json        # version 0.18.1, real commit, real built
```
...
**Homebrew** — only after the `chore: sync Homebrew formula checksums for v0.18.1`
...
before 0.18.1, so it is worth watching once.

**Self-update from the previous release:**

```bash
# on a machine still running 0.16.0
slmcode update --check      # must report v0.18.1 is available
slmcode update --yes
slmcode version             # 0.18.1
```
...
- Announce the **breaking behaviour changes**, not the feature list. For 0.18.1 those are:
...
```bash
git push --delete origin v0.18.1
git tag -d v0.18.1
