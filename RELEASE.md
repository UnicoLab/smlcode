...
For **v0.20.0 specifically**, the version and the changelog entry are already in the tree,
...
```bash
git show --stat HEAD
git tag -v v0.20.0 2>/dev/null || git show v0.20.0 --stat | head
```
...
git push origin v0.20.0          # this is what starts the release
```
...
|---|---|
| any `feat!:` / `BREAKING CHANGE:` | major — but **held inside 0.x** (0.26.0 → 0.26.0) |
| any `feat:` | minor (0.26.0 → 0.26.0) |
| only `fix:` / `perf:` | patch (0.26.0 → 0.24.1) |
...
```
slmcode_0.26.0_darwin_arm64      slmcode_0.26.0_windows_amd64.exe
slmcode_0.26.0_darwin_amd64      slmcode_0.26.0_windows_arm64.exe
slmcode_0.26.0_linux_arm64       install.sh  install.ps1  install.cmd
slmcode_0.26.0_linux_amd64       SHA256SUMS
```
...
```bash
cd "$(mktemp -d)"
curl -fsSLO https://github.com/UnicoLab/smlcode/releases/download/v0.26.0/SHA256SUMS
curl -fsSLO https://github.com/UnicoLab/smlcode/releases/download/v0.26.0/slmcode_0.26.0_darwin_arm64
shasum -a 256 -c SHA256SUMS --ignore-missing        # must say OK
chmod +x slmcode_0.26.0_darwin_arm64
./slmcode_0.26.0_darwin_arm64 version --json        # version 0.20.0, real commit, real built
```
...
**Homebrew** — only after the `chore: sync Homebrew formula checksums for v0.20.0`
...
- Announce the **breaking behaviour changes**, not the feature list. For 0.20.0 those are:
