...
// run's remaining runway: min(Timeout, RoleTimeout(base role)). It is what a
// site with no ctx to hand (reviewSlots builds slots before the race starts)
// uses; requestTimeout is the ctx-aware version every dispatch should use.
