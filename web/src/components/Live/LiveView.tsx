...
export default function LiveView() {
  ...
  useEffect(() => {
    ...
    getComposition().then((r) => setPersistedComposition(r.composition || null)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      previewComposition(q)
        .then((r) => {
          if (!cancelled) setCompositionPreview(r.composition || null);
        })
        .catch(() => {
          if (!cancelled) setCompositionPreview(null);
        })
        .finally(() => {
  ...
  const dynamicPhaseOrder = useMemo(() => {
    if (!shownComposition?.phases?.length) return null;
    return shownComposition.phases
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-800 glass-alt">
        ...
        {!running && interrupted.length > 0 && (
          <div className="mt-3 flex max-w-3xl flex-col gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100 sm:flex-row sm:items-center sm:justify-between">
      ...
      {(ctx?.config?.active_pack || ctx?.config?.active_pipeline || ctx?.config?.active_stack || running) && (
        <div className="px-4 py-1.5 glass-alt border-b border-gray-100 dark:border-gray-700 flex items-center gap-3 flex-wrap">
          ...
          {running && activeExecute && (
            <>
              <span className="text-[10px] text-gray-300">|</span>
              <span className="text-[10px] text-gray-400">worker:</span>
              <span className="badge-neutral text-[10px]">{activeExecute.default_role || 'worker'}</span>
              <span className="text-[10px] text-gray-400">review:</span>
              <span className="badge-neutral text-[10px]">{activeExecute.reviewer || 'reviewer'}</span>
              {(activeExecute.max_waves ?? 0) > 0 && (
                <span className="text-[10px] text-gray-400">waves: {activeExecute.max_waves}</span>
              )}
            </>
          )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs text-gray-600 dark:text-gray-300 truncate max-w-xl">
              ...
              <div className="flex flex-wrap gap-1.5">
                {shownComposition.phases
                  .filter((p) => p.enabled && p.when !== 'never')
                  .map((p, i) => (
                    <span key={`${p.id}-${i}`} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/80 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 text-[11px]">
              ...
              {shownComposition.team && shownComposition.team.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {shownComposition.team.map((member) => {
                    ...
                    return (
                      <span key={member.role} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-teal-100/80 dark:bg-teal-900/40 text-[11px] text-teal-900 dark:text-teal-100">
                        <Bot size={12} />
                        <span className="font-semibold">{spec?.title || member.role}</span>
                        {member.skills && member.skills.length > 0 && (
                          <span className="text-teal-700 dark:text-teal-300">{member.skills.join(', ')}</span>
                        )}
                      </span>
                    );
                  })}
